    ...
    config = function()
      require("markdown").setup({
        ...
        on_attach = function(bufnr)
          ...
          map({ 'n', 'i' }, '<M-l><M-;>', '<Cmd>MDListItemBelow<CR>', opts)
          -- map({ 'n', 'i' }, '<M-L><M-;>', '<Cmd>MDListItemAbove<CR>', opts)
          map('n', '<M-t>', '<Cmd>MDTaskToggle<CR>', opts)
          map('x', '<M-t>', ':MDTaskToggle<CR>', opts)
        end, -- (fun(bufnr: integer)) callback when plugin attaches to a buffer
      });
    end
  },
  -- {
